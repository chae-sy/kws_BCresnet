...
# model=BCResNets(base_c=1)
# model.to("cuda")

import torch

class BCResNetsWithDebug(BCResNets):
    def forward(self, x):
        print(f"Input shape: {x.shape}")
        
        # Head: (Conv-BN-ReLU)
        x = self.cnn_head(x)
        print(f"After cnn_head: {x.shape}")
        
        # Body: BC-ResBlocks
        for i, num_modules in enumerate(self.n):
            for j in range(num_modules):
                x = self.BCBlocks[i][j](x)
                print(f"After BCBlock[{i}][{j}]: {x.shape}")
        ...
        x = x.view(-1, x.shape[1])
        print(f"Final output shape (after view): {x.shape}")
        

        return x

model = BCResNetsWithDebug(base_c=8, num_classes=12)
    
# (batch_size, channels, height, width) 크기의 랜덤 텐서 생성
input_tensor = torch.randn(1, 1, 40, 61)  # 1개의 샘플, 1채널, 높이 128, 너비 64
